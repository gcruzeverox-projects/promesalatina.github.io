...
function NewQuoteContent() {
  ...
  async function handleSubmit(data: QuoteFormData) {
    ...
    try {
      ...
      router.push(`/${locale}/admin/quotes/${quote.id}?created=1`)
    } catch (e: any) {
  ...
  if (!orderId) return <div style={{ padding: 48, textAlign: 'center' }}><p style={{ color: '#EF4444' }}>No se especificó una orden.</p><Link href={`/${locale}/admin/quotes`} style={{ color: '#1F3A93' }}>← Volver</Link></div>
  if (loading)  return <div style={{ padding: 56, textAlign: 'center', color: '#94A3B8' }}>Cargando orden...</div>
  if (!order)   return <div style={{ padding: 48, textAlign: 'center' }}><p style={{ color: '#EF4444' }}>Orden no encontrada.</p><Link href={`/${locale}/admin/quotes`} style={{ color: '#1F3A93' }}>← Volver</Link></div>
  ...
  return (
    <>
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E2E8F0', padding: '28px 28px' }}>
        <QuoteEditor order={order} onSubmit={handleSubmit} onCancel={() => router.push(`/${locale}/admin/quotes`)} isSubmitting={isSubmitting} />
      </div>
    </>
  )
}
