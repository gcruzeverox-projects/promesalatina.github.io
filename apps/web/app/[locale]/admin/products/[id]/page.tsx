...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (!params.id) return
    getProduct(params.id).then(setProduct).catch(() => setError('Producto no encontrado')).finally(() => setLoading(false))
  }, [params.id])
  ...
  async function handleSubmit(data: ProductFormData) {
    ...
    setError(null)
    try {
      const updated = await updateProduct(product.id, toApiPayload(data))
      setProduct(updated)
      setSaved(true)
  ...
  return (
    <AdminLayout>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 24 }}>
        <div style={{ width: 52, height: 52, borderRadius: 10, overflow: 'hidden', background: '#F1F5F9', border: '1px solid #E2E8F0', flexShrink: 0 }}>
        ...
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, fontFamily: 'Poppins, sans-serif', color: '#0F172A', margin: 0 }}>{product.name}</h1>
          <p style={{ color: '#94A3B8', fontSize: 12, margin: '3px 0 0', fontFamily: 'monospace' }}>SKU: {product.sku}</p>
        </div>
        <div style={{ marginLeft: 'auto' }}>
          {saved && <span style={{ fontSize: 12, color: '#22C55E', fontWeight: 600 }}>✓ Guardado</span>}
        </div>
      </div>
      {justCreated && <div style={{ background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: '#166534' }}>✅ Producto creado. Ahora puedes subir imágenes en la pestaña <strong>Imágenes</strong>.</div>}
